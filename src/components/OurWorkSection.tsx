        ...
        <div className="relative flex-1 flex items-center my-auto">
          <motion.div ref={trackRef} style={{ x }} className="flex w-max gap-8 pl-4 pr-24">
            {PROJECTS.map((project) => (
              <div
                key={project.id}
                className="group relative h-[420px] w-[320px] sm:w-[440px] flex-shrink-0 bg-zinc-900/90 border border-zinc-800 rounded-2xl overflow-hidden p-6 flex flex-col justify-between hover:border-[#D4FF00] transition-all duration-300 shadow-[8px_8px_0px_0px_rgba(0,0,0,0.5)]"
              >
                ...
                <div className="relative z-10 flex justify-between items-center font-mono text-xs">
                  <span className="bg-[#D4FF00] text-black px-2.5 py-1 rounded font-bold">
                    [{project.id}]
                  </span>
                ...
                <div className="relative z-10 space-y-4">
                  <div>
                    <span className="text-[10px] font-mono uppercase text-[#D4FF00] tracking-wider">
                      {project.category}
                    </span>
                    <h3 className="text-2xl sm:text-3xl font-black uppercase tracking-tight font-['Cabinet_Grotesk'] text-white group-hover:text-[#D4FF00] transition-colors">
                      {project.title}
                    </h3>
                  </div>
                ...
                <div className="absolute top-6 right-6 z-10 w-10 h-10 rounded-full bg-black/60 border border-zinc-700 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 group-hover:bg-[#D4FF00] group-hover:text-black group-hover:border-[#D4FF00] transition-all duration-300 font-mono">
                  ↗
                </div>
              </div>
            ))}
